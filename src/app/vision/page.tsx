      ...
      <section style={{
        ...
        <div style={container}>
          ...
          <AnimatedSection delay={100}>
            <p style={{ fontSize: 18, fontWeight: 400, color: "var(--muted-foreground)", lineHeight: 1.8, maxWidth: 560, margin: 0 }}>
      ...
      <section style={{ ...sectionPad, backgroundColor: "var(--background)", borderBottom: "1px solid var(--border)" }}>
        <div style={container}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <AnimatedSection delay={0}>
              <div style={{ background: "#F8F2F4", borderRadius: 24, padding: "48px 40px", height: "100%", boxSizing: "border-box" as const }}>
                <div style={{ fontSize: 11, fontWeight: 400, color: "var(--primary)", marginBottom: 20 }}>Mission</div>
                <h3 style={{ fontSize: "clamp(22px, 2.5vw, 30px)", fontWeight: 700, color: "#1d1d1f", lineHeight: 1.3, margin: "0 0 16px" }}>
            ...
            <AnimatedSection delay={100}>
              <div style={{ background: "var(--primary)", borderRadius: 24, padding: "48px 40px", height: "100%", boxSizing: "border-box" as const }}>
                <div style={{ fontSize: 11, fontWeight: 400, color: "var(--muted-foreground)", marginBottom: 20 }}>Vision</div>
                <h3 style={{ fontSize: "clamp(22px, 2.5vw, 30px)", fontWeight: 700, color: "var(--foreground)", letterSpacing: "-0.024em", lineHeight: 1.3, margin: "0 0 16px" }}>
                  A world where no Indian woman manages her health alone.
                </h3>
                <p style={{ fontSize: 16, fontWeight: 400, color: "var(--muted-foreground)", lineHeight: 1.8, margin: 0 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" as const, gap: 2 }}>
            {roadmapItems.map((item, i) => (
              <AnimatedSection key={i} delay={i * 80}>
                <div style={{
                  background: "var(--card)",
                  borderRadius: i === 0 ? "8px 8px 0 0" : i === roadmapItems.length - 1 ? "0 0 8px 8px" : 0,
                  display: "grid", gridTemplateColumns: "160px 1fr", gap: 0,
                }}>
                  <div style={{ padding: "28px 0", borderRight: "1px solid rgba(246,24,135,0.1)", display: "flex", alignItems: "flex-start" }}>
                    <span style={{ fontSize: 13, fontWeight: 400, color: "var(--primary)", letterSpacing: "0.02em" }}>{item.period}</span>
                  </div>
                  <div style={{ padding: "28px 28px" }}>
                    <div style={{ fontSize: 16, fontWeight: 500, color: "var(--foreground)", marginBottom: 8, lineHeight: 1.4 }}>{item.headline}</div>
              ...
              <AnimatedSection key={i} delay={i * 60}>
                <div style={{
                  ...
                  <div style={{ padding: "28px 20px", borderRight: "1px solid rgba(246,24,135,0.1)", display: "flex", alignItems: "flex-start", justifyContent: "center" }}>
                    <span style={{ fontSize: 13, fontWeight: 400, color: "var(--primary)" }}>{v.num}</span>
                  </div>
                  <div style={{ padding: "28px 28px" }}>
                    <div style={{ fontSize: 17, fontWeight: 500, color: "var(--foreground)", marginBottom: 8 }}>{v.title}</div>
      ...
      <section style={{ ...sectionPad, backgroundColor: "#fbf9fb" }}>
        <div style={kit.container}>
          <AnimatedSection delay={0}>
            ...
            <h2 style={{ fontSize: "clamp(28px, 3.5vw, 40px)", color: "var(--foreground)", letterSpacing: "-0.018em", lineHeight: 1.25, margin: "0 0 24px" }}>
